...
def test_suggest_short_title_zotero_format():
    """Zotero 兜底短标题：无 comment → Unknown-updated年份-缩写"""
    from src.core.text import suggest_short_title

    title = suggest_short_title(
        {"updated": "2024-05-01", "title": "A Simple Framework for Contrastive Learning"}
    )
    assert title == "Unknown-2024-SimpLearni"
    assert "????" in suggest_short_title({"updated": "", "title": "A Novel Method"})
...
def sched():
    ...
    settings = SimpleNamespace(
        scheduler=SimpleNamespace(enabled=True, fetch_time="08:30", catch_up_on_start=True),
...
def test_scheduler_should_catch_up_after_time(sched, monkeypatch):
    ...
    async def _check():
        with patch("src.db.PaperDB") as db:
            ...
            assert await sched._should_catch_up() is True
            db.return_value.has_successful_since.return_value = True
            assert await sched._should_catch_up() is False
