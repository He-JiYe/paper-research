...
class TestCmdFetchIncremental:
    ...
    def test_fetch_with_apikey(self, conn, capsys):
        ...
        from src.config import (AppConfig, FetchConfig, LLMConfig,
...
class TestCmdServe:
    ...
    def test_serve_with_summary(self, conn, mock_settings, capsys, temp_dir):
        ...
        with patch("src.commands.OUTPUT_DIR", Path(temp_dir)):
            ...
            with patch("webbrowser.open"), patch("src.serve.server.run_server"):
...
class TestCmdReviewAll:
    ...
    def _setup(self, conn):
        ...
        insert_paper(conn, {'arxiv_id': '2401.00001', 'version': 1, 'title': 'Paper 1', 'authors': 'A', 'abstract': 'abs', 'url': '', 'primary_category': 'cs.LG', 'categories': 'cs.LG', 'published': '2024-01-01', 'arxiv_updated': '', 'keyword_match': 'test', 'fetch_date': '2024-07-01'})

        update_paper_summary(conn, '2401.00001', 'S', 'important', 'R', 0.95)



    def test_review_all(self, conn, mock_settings, capsys):
...
class TestCmdListMore:
    ...
    def test_list_rdate(self, setup_sample_papers, conn, mock_settings, capsys):
    ...
    def test_list_rscore(self, setup_sample_papers, conn, mock_settings, capsys):
    ...
    def test_list_status(self, setup_sample_papers, conn, mock_settings, capsys):
...
class TestCmdNoteForce:
    ...
    def test_note_force_regenerate(self, conn, mock_settings, capsys, sample_papers):
        ...
        insert_paper(conn, {'arxiv_id': '2401.00001', 'version': 1, 'title': 'Sector Rotation by Factor Model', 'authors': 'A', 'abstract': 'abs', 'url': '', 'primary_category': 'cs.LG', 'categories': 'cs.LG', 'published': '2024-01-01', 'arxiv_updated': '', 'keyword_match': 'test', 'fetch_date': '2024-07-01'})

        update_paper_summary(conn, '2401.00001', 'S', 'useful', 'R', 0.7)

        # Create a note dir with existing note to trigger 'Regenerating'
        ...
        (note_dir / 'note.md').write_text('old note', encoding='utf-8')
...
class TestCmdListAll:
    """cmd_list with --all flag"""

    def test_list_all(self, setup_sample_papers, conn, mock_settings, capsys):
        from src.commands import cmd_list

        cmd_list(args(command="list", all=True), mock_settings, conn)
        out = capsys.readouterr().out
        assert "2401" in out


class TestCmdMarkFetchFromArxiv:
    """cmd_mark fetch from arxiv when paper not in DB"""

    def test_mark_fetch_from_arxiv(self, conn, mock_settings, capsys):
        from unittest.mock import AsyncMock, MagicMock, patch
        ...
        mock_source.fetch_by_ids = AsyncMock(return_value=[paper])

        with patch("src.network.factory.get_source", return_value=mock_source):
            with patch("src.agents.PaperScorer") as MockPS:
                m = MagicMock()
                m.score.return_value = MagicMock(
                    summary="S", remark="useful", reason="R", score=0.8
                )
                MockPS.return_value = m
                with patch("src.agents.note_agent.NoteAgent") as MockNA:

                    async def g(*a, **kw):
                        return None

                    MockNA.generate_from_arxiv_id = g
                    with (
                        patch("src.serve.renderer.generate_summary_html"),
                        patch("src.serve.renderer.generate_notes_index_html"),
                        patch("os.startfile"),
...
class TestCmdNotify:
    """cmd_notify full function"""

    def test_notify(self, conn, mock_settings, capsys):
        ...
        with patch("src.notify.send_windows_toast") as mock_toast:
            with patch("src.notify.send_email_if_configured") as mock_email:
                cmd_notify(args(command="notify"), mock_settings, conn)
                out = capsys.readouterr().out
...
class TestCmdStatusNoLogs:
    """cmd_status with no fetch logs"""

    def test_status_no_logs(self, conn, mock_settings, capsys):
        from src.commands import cmd_status

        cmd_status(args(command="status"), mock_settings, conn)
        out = capsys.readouterr().out
        assert "no fetch logs yet" in out
